################################################################################
# GROUP DEG per CELLTYPE (pseudobulk limma-voom)
...
# ========================
alpha               <- 0.1
min_cells_per_key   <- 5 #min number of cells to be included
...
tag <- paste0("GroupDE_celltype_subjectBatch_ge", min_cells_per_key, "cells_", date_tag)

# Combined CSV
write.csv(res, paste0(tag, "_ALL_CELLTYPES.csv"), row.names = FALSE)
...
openxlsx::saveWorkbook(wb, paste0(tag, "_BY_CELLTYPE.xlsx"), overwrite = TRUE)
