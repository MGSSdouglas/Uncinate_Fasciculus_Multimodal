...
cell_counts <- meta_all %>%
  group_by(SubjectID, clusters_broad) %>%
...
cluster_proportions <- cell_counts %>%
  ...
  dplyr::select(SubjectID, clusters_broad, Proportion)

# 5. Pivot wider: 1 row per subject, 1 column per cluster
subject_proportions <- cluster_proportions %>%
  pivot_wider(names_from = clusters_broad, values_from = Proportion, values_fill = 0)
...
# Save results
write.csv(results_wilcoxonsummary, "~/scratch/20250714_libs1-12B_analysis_v4/20250815_results_wilcoxonsummary_clusters_broadprops.csv", row.names = FALSE)
print("Results saved")
...
combined_plot <- ggarrange(
  plotlist = plots_list, ncol = 3, nrow =3,
  common.legend = TRUE,
  legend = "none")
...
# Save
ggsave("~/scratch/20250714_libs1-12B_analysis_v4/figures/combined_proportion_broad.pdf", combined_plot, width = 12, height = 8)
