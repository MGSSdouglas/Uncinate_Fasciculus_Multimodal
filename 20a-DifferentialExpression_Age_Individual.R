################################################################################
# AGE EFFECT per INDIVIDUAL CELLTYPE (pseudobulk limma-voom)
...
# ========================
alpha               <- 0.10
min_cells_per_sbid  <- 5 #min number of cells to be included
...
plot_output_dir        <- "AgeEffect_PLOTS_AgeScatter_CELLTYPE"
...
drop_mask <- !is.na(colData(sce)$cluster_id) & 
  !(colData(sce)$cluster_id %in% c("Mixed1", "Mixed2"))
sce <- sce[, drop_mask]
...
celltypes <- sort(unique(as.character(colData(sce)$cluster_id)))

for (cl in celltypes) {
  message("Analyzing celltype: ", cl)
...
tag <- paste0("AgeDE_celltype_subjectBatch_ge", min_cells_per_sbid,
              "cells_minSubj", min_subjects_needed,
              "_QW", use_quality_weights,
              "_GlobalAge_3pass_", date_tag)

# Combined CSV
write.csv(res_lin, paste0(tag, "_ALL_CELLTYPES.csv"), row.names = FALSE)

# Excel workbook (one sheet per celltype)
...
openxlsx::saveWorkbook(wb_lin, paste0(tag, "_BY_CELLTYPE.xlsx"), overwrite = TRUE)
...
message("  - ", tag, "_ALL_CELLTYPES.csv")
message("  - ", tag, "_BY_CELLTYPE.xlsx")
